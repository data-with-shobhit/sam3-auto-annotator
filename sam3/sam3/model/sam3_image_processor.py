...
class Sam3Processor:
    ...
    def add_point_prompt(self, points: List, labels: List, state: Dict):
        """Adds point prompts and runs inference.
        points: List of [x, y] in range [0, 1]
        ...
        if "language_features" not in state["backbone_out"]:
             # If no text prompt, set dummy "visual" prompt
            dummy_text_outputs = self.model.backbone.forward_text(
                ["visual"], device=self.device
            )
            state["backbone_out"].update(dummy_text_outputs)
        ...
        # Format points: (N, 2) -> (N, B=1, 2)
        # Format labels: (N,) -> (N, B=1)
